    ...
    def evaluate(
        ...
        try:
            ...
            try:
                ...
                print_with_color(f"⚠️ JSON parsing failed: {e}", "red")
                result = {"complete": "maybe", "sub_scores": {}, "reason": "Could not parse evaluation result."}
                cost = 0.0

            return result, cost

        except IndexError as e:
            print_with_color(f"⚠️ Evaluation skipped due to missing log data: {e}", "yellow")
            return {"complete": "maybe", "sub_scores": {}, "reason": "No AppAgent log available."}, 0.0
